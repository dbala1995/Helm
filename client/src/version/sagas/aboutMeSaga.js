...
const getLastResponse = async () => {
  ...
  if (response.status === 200) {
    ...
    if (result.entry.length > 0) return obtainLatestResponses(result)
    else return null
  } else {
    return null
  }
      ...
      synopsis: response
        ? [
            {
              text: (
                <React.Fragment>
                  <Typography>4 questions designed to help you reflect on what matters to you.</Typography>
                </React.Fragment>
              ),
            },
            {
              text: (
                <React.Fragment>
                  <Typography>Information last submitted ({response[0].date})</Typography>
                </React.Fragment>
              ),
            },
            {
              text: (
                <React.Fragment>
                  {response.map((obj) => (
                    <div>
                      <Typography>{String(obj.question)}</Typography>
                      <Typography>{"Answer: " + String(obj.answer)}</Typography>
                    </div>
                  ))}
                </React.Fragment>
              ),
        ...
        : [
            ...
            },
            {
              text: (
                <React.Fragment>
                  <Typography>No previous entries.</Typography>
                </React.Fragment>
              ),
            },
          ],
